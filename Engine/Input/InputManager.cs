...
{
    /// <summary>
    /// Input Manager class from the Monocle Engine from Maddy Thorson<br/>
    /// Might include some minor changes<br/>
    /// </summary>
    public static class InputManager
    {
        ...
        public static bool _disabled = false;
        ...
        {
            foreach (var virtualInput in _virtualInputs)
                virtualInput.Update();
        }
        ...
        public class MouseData
        {
            ...
            public Point Position
            {
                get
                {
                    return Tools.InverseTransform(new Vector2(CurrentState.X, CurrentState.Y));
                }

                set
                {
                    Vector2 vector = Tools.Transform(value);
                    Microsoft.Xna.Framework.Input.Mouse.SetPosition((int)vector.X, (int)vector.Y);
                }
            }
            ...
            public Point deltaPosition
            {
                get
                {
                    return Tools.InverseTransform(new Vector2(CurrentState.X, CurrentState.Y)) - Tools.InverseTransform(new Vector2(PreviousState.X, PreviousState.Y));
                }
