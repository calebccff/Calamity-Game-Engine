...
{
    /// <summary>
    /// The Character class is the base class for all characters in the game.<br/>
    /// It has CharacterComponents that can be added to it, handling all of the logic and rendering for that character<br/>
    /// </summary>
    public abstract class Character : DrawableComponent, ICharacter
    {
        private Point __position = new Point();
        /// <summary>
        /// The position of the character<br/>
        /// It should be set only by functions with the tag "MoverTag"<br/>
        /// This will be used later to handle game logic<br/>
        /// </summary>
        public Point Position
        {
            ...
            set
            {
                ...
                // Get type of class declaring method
                Type type = method.DeclaringType;

                // Threw exception if neither the declaring class or the method has the tag "MoverTag"
                if (type.IsAssignableTo(typeof(ITaggable)) && !Functions.Tag<MoverTag>(type) && !Functions.Tag<MoverTag>(method)) throw new AccessViolationException(type.Name + " tried to move a character of type " + this.GetType().Name + " with method " + method.Name);

                // If it has the tag, do the move
                // Save old position
                // Set the underlying position of the component as well
                Point old = __position; _position = value; __position = value;

                // Invoke event __onPositionChange with old and new position
                Events._onPositionChange.Invoke(old, value);

            }
        }

        /// <summary>
        /// The collection of events for the character<br/>
        ...
        public string State
        {
            ...
            set
            {
                ...
                else throw new ArgumentException(value + " is not a member of the States of character " + base._id);
            }
        }
